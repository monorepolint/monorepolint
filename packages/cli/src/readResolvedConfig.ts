...
export async function readResolvedConfig(host: Host, args: Options) {
  ...
  for (const configPath of configFilesToTry) {
    ...
    try {
      ...
      throw new Error(
        `File exists ('${foundConfig}') but could not be imported due to error: ${
  ...
  if (unverifiedConfig === undefined) {
    if (foundConfig) {
      throw new Error(
        `File exists ('${foundConfig}') and was imported but the default export was undefined`,
      );
    } else {
      throw new Error(
  ...
  if (typeof unverifiedConfig === "function") {
    const tmpContext = await createWorkspaceContext(host, process.cwd(), {
      ...args,
      rules: [],
    });
