...
In earlier versions of monorepolint, the configuration file was named `monorepolint.config.ts`. Due to difficulties in maintaining TypeScript configurations that checked properly, particularly in Visual Studio Code, this format has been removed.
